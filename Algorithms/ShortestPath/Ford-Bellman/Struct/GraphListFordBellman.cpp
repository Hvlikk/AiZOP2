...
#include "GraphListFordBellman.h"
// GraphListPrim.cpp
#include <vector>
#include <queue>
#include <functional>
...
// Constructor
GraphListFordBellman::GraphListFordBellman(int vertices) : V(vertices) {
    adjacencyList.resize(V);
    std::srand(std::time(0)); // Initialize random number generator
}

// Add edge to the graph
void GraphListFordBellman::addEdge(int src, int dest, int weight) {
...
void GraphListFordBellman::generateSpanningTree() {
...
void GraphListFordBellman::generateAdditionalEdges(double density) {
...
void GraphListFordBellman::printAdjacencyList() const {
    for (int i = 0; i < V; ++i) {
        std::cout << i << ": ";
...
std::vector<std::tuple<int, int, int>> GraphListFordBellman::getEdges() const {
...
bool GraphListFordBellman::isEdgePresent(int u, int v) const {
...
void GraphListFordBellman::fordBellman(int startVertex, int endVertex) {
